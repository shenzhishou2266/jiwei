...
namespace 积微.Controls
{
    ...
    public partial class FocusSessionControl : SWC.UserControl
    {
        ...
        private void UserControl_PreviewKeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            ...
            {
                if (_timerService.CurrentGoal != null)
                {
                    e.Handled = true;
                    ShowTimelineInput();
                }
        ...
        private async System.Threading.Tasks.Task SaveTimelineEntry()
        {
            ...
            if ((!string.IsNullOrEmpty(text) || _previewImages.Count > 0) && _timerService.CurrentGoal != null)
            {
                ...
                {
                    _timerService.CurrentGoal.AddTimelineEntry(text, type, null, imagePathList);
                }
                else
                {
                    _timerService.CurrentGoal.AddTimelineEntry(text, type);
                }
                await DataStorageService.SaveGoalsAsync(GoalsPage.Goals);
            }
            HideTimelineInput();
        ...
        private string GetImageStoragePath()
        {
            ...
            {
                dataStoragePath = System.IO.Path.Combine(
                    Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                    "积微"
                );
            }
            
            string imagePath = System.IO.Path.Combine(dataStoragePath, "images");
        ...
        private string SaveImageToFile(SWI.BitmapSource bitmapSource)
        {
